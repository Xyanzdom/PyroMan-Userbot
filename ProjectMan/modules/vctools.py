...
async def end_vc_(client: Client, message: Message):
    ...
    await edit_or_reply(message, f"❌ Obrolan suara di hentikan **Group ID** : `{chat_id}`")
...
async def joinvc(client: Client, message: Message):
    ...
    await Man.edit(f" ☑️**menaiki obrolan suara**\n└ **Group ID:** `{chat_id}`")
    await sleep(5)
...
async def leavevc(client: Client, message: Message):
    ...
    msg = "❌ **Meninggalkan obrolan suara**"
    if chat_id:
        msg += f"\n└ **Group ID:** `{chat_id}`"
    await Man.edit(msg)
